...
class Pathfinder:
    ...
    def astar_path_length(self, start, goal):
        ...
        self.goal = goal
        graph = self.create_graph(self.game_map)
        return astar.astar_path_length(graph, start, goal)
    ...
    def parse_game_state_first_turn(self, game_state):
        game_map = [[]]
        y = 0
        x = 0
        for character in game_state[:-1]:
            if character == '\n':
                ...
                game_map[y].append(self.create_symbol(character))
                if character == str(ObjectSymbols.JUNK.value):
                    self.junks.append((y,x))
                elif character == str(ObjectSymbols.SPIKE.value):
                    self.spikes.append((y,x))
                elif character == str(MapSymbols.GRASS.value):
                    self.grassList.append((y,x))
                x += 1
        return game_map
